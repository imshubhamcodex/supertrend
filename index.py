...
def st_changes(data):
    ...
    send_img(filename)
    send_message("Hi, SuperTrend Direction Changed on NSEI [15 min]")
...
def check_dates(data_frame, st_changes, data):
    ...
    if today in data_frame.index or yesterday in data_frame.index or day_before_yesterday in data_frame.index:
        st_changes(data)
